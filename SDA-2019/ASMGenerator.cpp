...
namespace ASMGenerator {
	...
	void AddConstInfo(Tables table) {
		...
		for (int i = 0; i < table.LEXTABLE->size; i++) {
			...
			if (table.LEXTABLE->table[i].lexema == LEX_LITERAL && table.IDTABLE->table[table.LEXTABLE->table[i].idxTI].littype == 3) {
				asmFile << "\t" << table.IDTABLE->table[table.LEXTABLE->table[i].idxTI].id << " SDWORD " << table.IDTABLE->table[table.LEXTABLE->table[i].idxTI].value.vint <<"h"<< endl;
			}
		}
	}
		...
		for (int i = 0; i < table.LEXTABLE->size; i++) {
			...
			case LEX_OUT:
				...
				if (table.IDTABLE->table[table.LEXTABLE->table[i + 2].idxTI].littype == 2) {
					asmFile << "\tpush offset " << table.IDTABLE->table[table.LEXTABLE->table[i + 2].idxTI].id;
					asmFile << "\n\tcall outstr\n";
					name.clear();
				}
				break;
			...
			case LEX_EQUAL:
				...
				while (table.LEXTABLE->table[i].lexema != LEX_SEMICOLON) {
					if (table.LEXTABLE->table[i].lexema == LEX_ID&& table.LEXTABLE->table[i+1].lexema!=LEX_COMMA && table.LEXTABLE->table[i - 1].lexema != LEX_COMMA&& table.LEXTABLE->table[i +1].lexema != LEX_LEFTHESIS) {
						isPol = false;
						for (int j = 0; j < 4; j++) {
					...
					if ((table.LEXTABLE->table[i-1].lexema == LEX_ID|| table.LEXTABLE->table[i - 1].lexema == LEX_LITERAL)&& table.LEXTABLE->table[i].lexema == LEX_RIGHTHESIS) {
						position = i;
