...
namespace  Lexer {
	...
	void Print(Tables tables)
	{
		...
		for (int i = 0; i < tables.IDTABLE->size; i++)
		{
			...
			it << setw(3) << i << ". Идентификатор: " << setw(6) << tables.IDTABLE->table[i].id << ". Тип: " << setw(3) << tables.IDTABLE->table[i].idtype << ". Значение: " << "Тип переменной :" << tables.IDTABLE->table[i].iddatatype;
